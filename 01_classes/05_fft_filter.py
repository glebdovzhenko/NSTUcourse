...
class FFTWorker:
    ...
    def set_ys_fft(self, max_freq=None):
        ...
        # копируем Фурье-образ, заполняем нулями высокие частоты, делаем обратное преобразование
...
class PlotWidget(QWidget, FFTWorker):
    ...
    def on_btn_open_pressed(self):
        # запускаем файловый диалог, получаем имя файла
        fd = QFileDialog(parent=self)
        fname, _ = fd.getOpenFileName()

        # поскольку имя файла может быть неверным (как и его содержимое),
        # функция np.loadtxt необязательно сработает
        # чтобы программа не завершилась с ошибкой, оборачиваем функцию для поимки исключений
        # прочитанные данные используем для заполнения в FFTWorker.set_data
        try:
